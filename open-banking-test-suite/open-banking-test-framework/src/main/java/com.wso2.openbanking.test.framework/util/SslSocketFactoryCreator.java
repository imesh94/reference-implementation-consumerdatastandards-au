...
public class SslSocketFactoryCreator {
     ...
     * @throws TestFrameworkException when an error occurs while loading the keystore and truststore
     */
    public SSLSocketFactory createForMockCDRRegister() throws TestFrameworkException {
        try (FileInputStream keystoreLocation =
                     new FileInputStream(new File(ConfigParser.getInstance().getTransportKeystoreLocationForMockCDRRegister()));
             FileInputStream truststoreLocation =
                     new FileInputStream(new File(ConfigParser.getInstance().getTransportTruststoreLocationForMockCDRRegister()))) {

            KeyStore keyStore = KeyStore.getInstance(ConfigParser.getInstance().getTransportKeystoreTypeForMockCDRRegister());
            keyStore.load(keystoreLocation, ConfigParser.getInstance().getTransportKeystorePasswordForMockCDRRegister()
                    .toCharArray());
            KeyStore trustStore = KeyStore.getInstance(ConfigParser.getInstance().getTransportTruststoreTypeForMockCDRRegister());
            trustStore.load(truststoreLocation, ConfigParser.getInstance().getTransportTruststorePasswordForMockCDRRegister()
                    .toCharArray());

            return new org.apache.http.conn.ssl.SSLSocketFactory(keyStore, ConfigParser.getInstance()
                    .getTransportKeystorePasswordForMockCDRRegister(), trustStore);
