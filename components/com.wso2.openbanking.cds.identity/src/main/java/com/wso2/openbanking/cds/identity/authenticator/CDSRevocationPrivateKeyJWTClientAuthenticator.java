...
public class CDSRevocationPrivateKeyJWTClientAuthenticator extends CDSBasePrivateKeyJWTClientAuthenticator {
    ...
    private static final Log log = LogFactory.getLog(CDSRevocationPrivateKeyJWTClientAuthenticator.class);

    @Override
    public boolean canAuthenticate(HttpServletRequest httpServletRequest, Map<String, List> bodyParameters,
                                   OAuthClientAuthnContext oAuthClientAuthnContext) {

        if (httpServletRequest.getRequestURI().contains(Constants.REVOKE)) {
            log.debug("Request can be handled by CDSRevocationPrivateKeyJWTClientAuthenticator");
            return canSuperAuthenticate(httpServletRequest, bodyParameters, oAuthClientAuthnContext);
        }
        log.debug("CDSRevocationPrivateKeyJWTClientAuthenticator cannot handle the request.");
