...
public class CDSIntrospectionPrivateKeyJWTClientAuthenticator extends CDSBasePrivateKeyJWTClientAuthenticator {
    ...
    private static final Log log = LogFactory.getLog(CDSIntrospectionPrivateKeyJWTClientAuthenticator.class);

    @Override
    public boolean canAuthenticate(HttpServletRequest httpServletRequest, Map<String, List> bodyParameters,
                                   OAuthClientAuthnContext oAuthClientAuthnContext) {

        if (httpServletRequest.getRequestURI().contains(Constants.INTROSPECT)) {
            log.debug("Request can be handled by CDSIntrospectionPrivateKeyJWTClientAuthenticator");
            return canSuperAuthenticate(httpServletRequest, bodyParameters, oAuthClientAuthnContext);
        }
        log.debug("CDSIntrospectionPrivateKeyJWTClientAuthenticator cannot handle the request.");
