...
public class CDSConsentAdminHandler implements ConsentAdminHandler {
    ...
    public void handleSearch(ConsentAdminData consentAdminData) throws ConsentException {
        ...
        public void handleRevoke(ConsentAdminData consentAdminData) throws ConsentException {
            try {
                ...
                if (StringUtils.isBlank(consentID)) {
                    throw new ConsentException(ResponseStatus.BAD_REQUEST, "Mandatory parameter consent ID not " +
                            "available");
                } else {
                    final String userID = validateAndGetQueryParam(queryParams, USER_ID);
                    DetailedConsentResource detailedConsentResource = this.consentCoreService.getDetailedConsent
                            (consentID);
        ...
        @Override
        public void handleConsentAmendmentHistoryRetrieval (ConsentAdminData consentAdminData)
            ...
            try {
                ...
                if (isActionByPrimaryUser(currentConsentResource, userID)) {
                    ...
                    for (Map.Entry<String, ConsentHistoryResource> result : results.entrySet()) {
                        ...
                        consentResourceJSON.appendField("amendedTime", detailedConsentResource.
                                getUpdatedTime());
         ...
         */
        private void deactivateAccountMappings (DetailedConsentResource detailedConsentResource, String
        secondaryUserID)
                    ...
                    .ifPresent(auth ->
                            ...
                            consentMappings.stream()
                                    ...
                                    .forEach(secondaryAccountID ->
                                            // add every mapping resource that has secondary user's accountID to
                                            // mappingIds
                                            consentMappings.stream()
        ...
        private JSONObject detailedConsentToJSON (DetailedConsentResource detailedConsentResource) {
            ...
            try {
                ...
                log.error(String.format("Error occurred while parsing receipt in consent history. %s",
                        e.getMessage()));
